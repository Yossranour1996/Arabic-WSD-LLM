...
SENSE_REGEX = re.compile(r"""
    (?ixs)                    # i = case-insensitive, x = verbose, s = dot-all
    (?:                       # ── any ONE of the templates below ───────────
      ...
      | # 9  correct Sense ID for … is  <N>   (N may be bare, or **N**, or “Sense ID: N”)
       ...
       | # 12 I would choose: [optional newline] then number in any format
        ...
        (\d+)                                            # ← the number we want to capture
        \s* \**  
      ...
      | # 17 the correct sense for ... is: Sense ID N
        .*?the \s+ correct \s+ sense \s+ for \b .*? \s+ is \s* :? \s*
  \**? \s*                                # optional **
  (?: sense \s+ id \s* :? \s* )?          # optional "Sense ID:"
  (\d+)                                   # capture the number
  \s* \**?                                # optional **
 |  #18 the correct sense is Sense ID N
    .*? the \s+ correct \s+ sense \s+ is \s* :? \s*
        (?:sense \s+ id \s* :? \s*)?   # “Sense ID” literal
        \**? \s* (\d+) \s* \**?        # capture N, optional **
...
| #23  Therefore, the correct Sense ID is: N / **N**
  ...
  \**? \s* (\d+) \s* \**?
      | # 24  I would choose the correct sense ID as N (with optional punctuation/text)
        .*? i \s+ would \s+ choose \s+ the \s+ correct \s+ sense \s+ id \s+ as \b 
        .*? \s* \** \s* (\d+) \s* [.,]? \s* \**? [^.\d]*

      | # 25  I would choose the correct sense ID as: [newline] N (with optional punctuation)
        .*? i \s+ would \s+ choose \s+ the \s+ correct \s+ sense \s+ id \s+ as \s* : \s* 
        (?: \n \s* )?                                 # Optional newline
        \**? \s* (\d+) \s* [.,]? \s* \**? [^.\d]*
          | # 26  I would choose: ... Correct Sense ID: N
        .*? i \s+ would \s+ choose \s* : \s* 
        (?: \s | \n )*?       # optional whitespace or newline
        correct \s+ sense \s+ id \s* :? \s* 
        \**? \s* (\d+) \s* \**?
            | # 27  I'll choose Sense ID: N (with optional ** and punctuation)
        .*? i'?ll \s+ choose \s+ sense \s+ id \s* : \s* \** \s* (\d+) \s* \** [.,]?
        
    | # 28  I would choose: (optional newline) **Sense ID: N**
        .*? i \s+ would \s+ choose \s* : \s* 
        (?: \n \s* )? 
        \** sense \s+ id \s* : \s* (\d+) \s* \** [.,]?
        
    | # 29  I would choose: (optional newline) Correct Sense ID: N
        .*? i \s+ would \s+ choose \s* : \s* 
        (?: \n \s* )? 
        correct \s+ sense \s+ id \s* : \s* \**? \s* (\d+) \s* \**? [.,]?
        
    | # 30  I will choose Sense ID: N (with optional punctuation)
        .*? i \s+ will \s+ choose \s+ sense \s+ id \s* : \s* \**? \s* (\d+) \s* \**? [.,]?
        
    | # 31  I will choose: (optional newline) Sense ID: N
        .*? i \s+ will \s+ choose \s* : \s* 
        (?: \n \s* )? 
        \**? \s* sense \s+ id \s* : \s* \**? \s* (\d+) \s* \**? [.,]?
        
    | # 32  Correct Sense ID would be: N
        .*? the \s+ correct \s+ sense \s+ id \s+ would \s+ be \s* : \s* \**? \s* (\d+) \s* \**? [.,]?
        
    | # 33  I will choose [text]: (optional newline) Sense ID: N
        .*? i \s+ will \s+ choose \b .*? \s* : \s* 
        (?: \n \s* )? 
        \**? \s* sense \s+ id \s* : \s* \**? \s* (\d+) \s* \**? [.,]?
        
    | # 34  Correct sense [text] would be: (optional newline) Sense ID: N
        .*? the \s+ correct \s+ sense \b .*? \s+ would \s+ be \s* : \s* 
        (?: \n \s* )? 
        \**? \s* sense \s+ id \s* : \s* \**? \s* (\d+) \s* \**? [.,]?
